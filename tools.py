...
from ditic_kanban.rt_api import add_comment
from ditic_kanban.rt_api import new_ticket
...
def calculate_time_worked(ticket_line):
    ...
    if last_time_worked.find(' minutes') > 0:
        last_time_worked = float(last_time_worked[:last_time_worked.find(' minutes')])
    elif last_time_worked.find(' hours') > 0:
        last_time_worked = float(last_time_worked[:last_time_worked.find(' hours')])*60
    else:
        last_time_worked = 0
    try:
...
def create_new_ticket(rt_object, requestor, subject):
    result = new_ticket(
        rt_object,
        {
            'id': 'ticket/new',
...
def ticket_actions(rt_object, ticket_id, action, commentary, ticket_email, user_email):
    """
